...
type RoomProps = React.ComponentPropsWithoutRef<"group">;

export function Room(props: RoomProps) {
  const { nodes, materials } = useGLTF(
    "/assets/models/optimized-room.glb"
  ) as unknown as {
  ...
  const matcapTexture = useTexture("/assets/images/textures/mat1.png");

  const curtainMaterial = new MeshPhongMaterial({ color: "#d90429" });
  const bodyMaterial = new MeshPhongMaterial({ map: matcapTexture });
  const tableMaterial = new MeshPhongMaterial({ color: "#582f0e" });
  const radiatorMaterial = new MeshPhongMaterial({ color: "#fff" });
  const compMaterial = new MeshStandardMaterial({ color: "#fff" });
  const pillowMaterial = new MeshPhongMaterial({ color: "#8338ec" });
  const chairMaterial = new MeshPhongMaterial({ color: "#000" });
